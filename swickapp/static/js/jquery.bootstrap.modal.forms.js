...
(function ($) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $.ajax({
                            ...
                            dataType: "json",
                            success: function (response) {
                                // SWICK MODIFICATION: Restricting assignment to only be compatible with select customization_objects
                                // Must change for other types
                                var select = $(asyncSettings.dataElementId);
                                var options_arr = response[asyncSettings.dataKey]
                                //$(asyncSettings.dataElementId).html(response[asyncSettings.dataKey]);
                                select.empty()
                                for(var i = 0; i < options_arr.length; ++i){
                                  var name = options_arr[i][0];
                                  var percent = options_arr[i][1];

                                  var name_percent = name.concat(" (", percent, "%)");
    ...
    $.fn.modalForm = function (options) {
        ...
        this.each(function () {
            // Add change event handler to the element with attached modalForm
            $(this).change(function (event) {
                // SWICK MODIFICATION
                var select_object = $('#tax_categories'); // select_object = this
                var value = select_object.prop('options')[select_object.prop('selectedIndex')].value;
                var prev_index = select_object.data('prev');
                var curr_index = select_object.prop('selectedIndex');

                // if select option is "add_tax_category"
                if(value == "add_tax_category"){
                  select_object.prop('selectedIndex', prev_index);
                  // Instantiate new form in modal
                  modalForm(settings);
                }
                else{
                  select_object.data('prev', curr_index);
                }
            });
        });
